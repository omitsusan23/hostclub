...
export default function TableStatusPage() {
  ...
  return (
    <>
      ...
      {/* フィルターバー */}
      <div className="flex justify-between items-center p-4 bg-gray-100 space-x-2">
        ...
        <div className="ml-auto flex space-x-1">
          <button onClick={() => setGridCols(2)} className={gridCols===2?'font-bold':'text-gray-600'}>2列</button>
          <button onClick={() => setGridCols(3)} className={gridCols===3?'font-bold':'text-gray-600'}>3列</button>
        </div>
      </div>

      <main id="main-content" className={`p-4 grid gap-4 grid-cols-${gridCols}`}> 
        {renderedTables}
      </main>
