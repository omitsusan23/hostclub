...
export default function TableStatusPage() {
  ...
  const confirmFirst = () => {
    ...
    // 2) firstLabels 更新 & localStorage 保存
    setFirstLabels(prev => {
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <div className="mb-4 flex items-center space-x-4 justify-center">
              <label className="inline-flex items-center space-x-1">
                <input type="radio" name="firstType" value="初回" checked={firstType==='初回'} onChange={()=>setFirstType('初回')} />
                <span className="text-sm">初回</span>
              </label>
              <label className="inline-flex items-center space-x-1">
                <input type="radio" name="firstType" value="初回指名" checked={firstType==='初回指名'} onChange={()=>setFirstType('初回指名')} />
                  ...
                  {names.map((_, i)=>(
                    <div key={i}>
                      {positionLabelsByCount[selectedCount][i] && (
                        <div className="text-center text-sm text-gray-500 mb-1">{positionLabelsByCount[selectedCount][i]}</div>
                      )}
                      <input type="text" placeholder="名前" value={names[i]} onChange={e=>{
                        const a=[...names]; a[i]=e.target.value.slice(0,6); setNames(a);
                      }} className="border p-2 rounded w-full mb-1" />
                      <select value={photos[i]} onChange={e=>{
                        const b=[...photos]; b[i]=e.target.value; setPhotos(b);
