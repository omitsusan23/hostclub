...
export default function TableStatusPage() {
  ...
  return (
    <>
            ...
            <div className="mb-4 flex items-center space-x-4 justify-center">
              <label className="inline-flex items-center space-x-1">
                <input type="checkbox" checked={firstInitial} onChange={() => {
                  const v = !firstInitial;
                  if (!v && !firstDesignation) return;
                  setFirstInitial(v);
                }} />
              ...
              <label className="inline-flex items-center space-x-1">
                <input type="checkbox" checked={firstDesignation} onChange={() => {
                  const v = !firstDesignation;
                  if (!v && !firstInitial) return;
                  setFirstDesignation(v);
                }} />
            ...
            {step1 ? (
              <> 
                ...
                <div className="flex justify-end space-x-2">
                  <button onClick={closeFirstModal} className="px-4 py-2 bg-gray-300 rounded">キャンセル</button>
                  <button onClick={nextStep} disabled={!selectedTable || selectedCount<1} className="px-4 py-2 bg-blue-500 text-white rounded disabled:opacity-50">次へ</button>
                </div>
              </>
            ) : (
              <>
                ...
                {/* ここに名前/指名フィールド等を追加可能 */}
                <div className="flex justify-end space-x-2">
                  <button onClick={() => setStep1(true)} className="px-4 py-2 bg-gray-300 rounded">戻る</button>
                  <button onClick={confirmFirst} className="px-4 py-2 bg-green-500 text-white rounded">反映</button>
                </div>
              </>
            )}
          </div>
      ...
      <Footer currentUser={state.currentUser} onOpenAddReservation={()=>{}} onOpenFirst={openFirstModal} />
    </>
  );
}
