...
<div className="bg-gray-200 px-2 py-1 flex items-baseline justify-between">
  <div className="flex items-baseline space-x-1">
    ...
    <span className="text-lg font-bold leading-none self-baseline">{table.tableNumber}</span>
    {isInitial ? (
      <span className="text-xs leading-none self-baseline">🔰</span>
  ...
  {table.princess && (
    <button
      ...
      className="text-red-500 hover:text-red-700 leading-none self-baseline"
    >
      🗑
    </button>
  )}
...
<div className="bg-gray-200 px-2 py-1 flex items-baseline justify-between">
  ...
  {table.princess && (
    <button
      onClick={e => { e.stopPropagation(); handleDelete(table.id); }}
      aria-label={`卓 ${table.tableNumber} を削除`}
      className="text-[10px] hover:text-red-700 leading-none self-baseline"
    >
      🗑
    </button>
  )}
