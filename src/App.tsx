...
function AppInner() {
  ...
  const confirmFirst = () => {
    ...
    // 2) localStorage に firstLabels を保存
    const saved = JSON.parse(localStorage.getItem('firstLabels') || '{}')
    const types = Array.from(new Set(firstTypes))
    saved[selectedTable] = types.join('/')
          ...
          <div className="min-h-screen flex flex-col pb-16">
            ...
            {firstModalOpen && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                {/* ...modal content unchanged except confirmFirst handles */}
              </div>
            )}
          </div>
        </Suspense>
