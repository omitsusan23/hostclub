...
function AppInner() {
  ...
  return (
    <Router>
      <ErrorBoundary>
        <a href="#main-content" className="sr-only focus:not-sr-only p-2 bg-blue-500 text-white">Skip to main content</a>
        <Suspense fallback={<div className="p-4 text-center">Loading…</div>}>
          <div className="min-h-screen flex flex-col pb-16">
            ...
            {currentUser && <Footer currentUser={currentUser} onOpenAddReservation={openResModal} onOpenFirst={openFirstModal} />}
                  ...
                  {step1 ? (
                    <>
                      ...
                      <select value={selectedTable} onChange={e=>setSelectedTable(e.target.value)} className="border p-2 w-full rounded mb-4">
                        ...
                        {tableSettings.map(t=> tables.some(tab=> tab.tableNumber===t) ? <option key={t} value={t} disabled>{t}（使用中）</option> : <option key={t} value={t}>{t}</option>)}
                      </select>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        {names.map((_,i)=>(
                          <div key={i}>
                            ...
                            <input type="text" placeholder="名前" value={names[i]} onChange={e=>{const a=[...names]; a[i]=e.target.value.slice(0,6); setNames(a)}} className="border p-2 rounded w-full mb-1" />
                            <select value={firstTypes[i]==='初回指名'? firstPhotos[i] : photos[i]} onChange={e=>{const arr= firstTypes[i]==='初回指名' ? [...firstPhotos] : [...photos]; arr[i]=e.target.value; firstTypes[i]==='初回指名' ? setFirstPhotos(arr) : setPhotos(arr)}} className="border p-2 rounded w-full mb-1">
                              {firstTypes[i]==='初回指名' ? <><option value="">指名してください</option>{casts.map(c=><option key={c} value={c}>{c}</option>)}</> : <><option value="なし">写真指名なし</option>{casts.map(c=><option key={c} value={c}>{c}</option>)}</>}
                            </select>
                          </div>
                        ))}
                      </div>
